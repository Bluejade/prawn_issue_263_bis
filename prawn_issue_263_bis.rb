...
Prawn::Document.generate "prawn_issue_263_example.pdf" do |pdf|
  ...
  pdf.table [["Following cell must be bold", "<b>PDF for ISSUE 263</b>"]], cell_style: { height: 60, :inline_format => true } do
  ...
  pdf.bounding_box [pdf.bounds.left, pdf.bounds.top - 82], width: pdf.bounds.width do
    ...
    question_cell = "<b>Which is the L value if L is the subject of the formulae 2πfL = x?</b>\n\nA L = 2πf\nB L = 2πf / x\nC L = x / 2πf"
    pdf.table [["1", question_cell]], cell_style: { height: 60, :inline_format => true }
  end
end
